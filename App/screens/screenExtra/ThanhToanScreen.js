...
const ThanhToanScreen = ({ route, navigation }) => {
  ...
  const [isPaymentSuccessModalVisible, setPaymentSuccessModalVisible] =
    useState(false);
  const [isPaymentFailureModalVisible, setPaymentFailureModalVisible] =
    useState(false);
  ...
  const calculateTotalAmount = () => {
    ...
    selectedProducts.forEach((item) => {
      const product = dataSP.find((product) => product._id === item.product);
      productSoLuong += item.quantity;
      productTotal += item.quantity * product.product_price;
    });
  ...
  const renderProductItem = (item) => {
    const product = dataSP.find((product) => product._id === item.product);
    return (
      <View key={item.sizeId} style={styles.productItem}>
        <Image
          source={{ uri: product.product_image }}
          style={styles.productImage}
        />
        <View style={styles.productDetails}>
          <Text>Giày: {product.product_title}</Text>
          <Text>Màu: {item.color}</Text>
          <Text>Kích thước: {item.size}</Text>
          <Text>Giá: {product.product_price}</Text>
  ...
  const handleOrder = async () => {
    if (!shippingAddress) {
      Alert.alert(
        "Hãy Thao Tác Lại",
        "Vui lòng chọn địa chỉ nhận hàng trước khi thanh toán."
      );
      return;
    }
    ...
    <View style={styles.container}>
      ...
      >
        <View>
          ...
          <Text style={styles.addressText}>
            Địa chỉ: {shippingAddress.label}: {shippingAddress.address}
          </Text>
        </View>
        <Icon name="globe" size={100} color="#1abc9c" />
      </TouchableOpacity>
      <ScrollView style={styles.scrollContainer}>
        {selectedProducts.map((product) => renderProductItem(product))}
      </ScrollView>
        ...
        <TouchableWithoutFeedback onPress={handlePaymentModalClose}>
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              ...
              <TouchableOpacity
                onPress={handleMomoCheckboxChange}
                style={styles.paymentIcon}
              >
                <Icon
                  name={isMomoSelected ? "check-circle" : "circle"}
                  size={30}
                  color="#000"
                  width={40}
                />
              ...
              <TouchableOpacity
                onPress={handleCODCheckboxChange}
                style={styles.paymentIcon}
              >
                <Icon
                  name={isCODSelected ? "check-circle" : "circle"}
                  size={30}
                  color="#000"
                  width={40}
                />
