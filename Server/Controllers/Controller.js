...
app.get("/product/:id", async (req, res) => {
  try {
    ...
    const sizeQuantity = sizes.reduce(
      (total, size) => total + size.size_quantity,
      0
    );
...
app.get("/cart", async (req, res) => {
  try {
    ...
    for (const cartItem of cartItems) {
      ...
      const color = await Color.findById(size.colorId);
      const product = await Product.findById(color.product);
      ...
      arrCartDetails.push(newItemCart);
    }
    res.json(arrCartDetails);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
app.post("/favourite/add", async (req, res) => {
  ...
  try {
    ...
    if (favouriteItem) {
      ...
      res
        .status(400)
        .json({ message: "Sản phẩm đã tồn tại trong bảng yêu thích" });
    } else {
...
app.post("/order/add", async (req, res) => {
  ...
  try {
    ...
    const foundProducts = await Product.find({
      _id: { $in: products.map((item) => item.product_id) },
    });
    ...
    if (foundProducts.length !== products.length) {
      ...
      for (const item of products) {
        const foundProduct = foundProducts.find((product) =>
        ...
        if (!size) {
          res.status(404).json({
            message: "Không tìm thấy kích thước phù hợp cho sản phẩm",
          });
          return;
        }
...
app.get("/products/search", async (req, res) => {
  ...
  try {
    ...
    const products = await Product.find({
      product_title: { $regex: searchString, $options: "i" },
    });
    res.json(products);
  } catch (error) {
    console.log(error);
